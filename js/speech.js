(async function() {
    ...
    // Try to load transformers.js for local model support
            ...
            async start() {
                ...
                try {
                    ...
                    this._recorder.onstop = async () => {
                        ...
                        const output = await this._transcriber(audioBuffer.getChannelData(0));
                        
                        if (this.onresult) {
                            const event = {
                                results: [
                                    Object.assign([
                                        { transcript: output.text, confidence: 1.0 }
                                    ], { isFinal: true })
                                ],
                                resultIndex: 0
                            };
    ...
    const SpeechEngine = LocalSpeechRecognition || window.SpeechRecognition || window.webkitSpeechRecognition || window.mozSpeechRecognition || window.msSpeechRecognition;
    if (!SpeechEngine) {
